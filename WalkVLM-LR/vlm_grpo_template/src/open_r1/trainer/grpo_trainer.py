...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # model_init_kwargs["use_cache"] = False  # Disable the cache
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, pixel_values, image_grid_thw):
        # 查看当前 tokenizer 的 padding_side
        # print(self.processing_class.tokenizer.padding_side)
        logits = model(input_ids, attention_mask=attention_mask, pixel_values=pixel_values, image_grid_thw=image_grid_thw, use_cache=False).logits  # (B, L, V)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 处理关键词
        keywords = [x['prompt'][-1]["content"][1]["keywords"] if "keywords" in x['prompt'][-1]["content"][1] else [] for x in inputs]

        
        # prompts_text = [apply_chat_template(example, self.processing_class)["prompt"] for example in inputs]
        prompts_text = ['<|vision_start|><|image_pad|><|vision_end|>' + example['prompt'][-1]['content'][-1]['text'] for example in inputs]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                reward_kwargs = {key: [] for key in inputs[0].keys() if key not in ["prompt", "completion", "keywords"]}
                for key in reward_kwargs:
                    for example in inputs:
                        # Repeat each value in the column for `num_generations` times
                        reward_kwargs[key].extend([example[key]] * self.num_generations)
                
                # keywords = [x["keywords"] for x in inputs]  # Extract 'keywords' field
                for example in keywords:
                # Repeat the 'keywords' field `num_generations` times
                    reward_kwargs["keywords"] = [example] * self.num_generations

                output_reward_func = reward_func(prompts=prompts, completions=completions, **reward_kwargs)
                # output_reward_func = reward_func(prompts=prompts, completions=completions, keywords=reward_kwargs["keywords"], **reward_kwargs)
                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)

        # Sum the rewards from all reward functions
        reward_weights = [1,1,1,1]
        weighted_rewards = rewards_per_func * torch.tensor(reward_weights, dtype=torch.float32, device=device)
